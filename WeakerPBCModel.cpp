...
//=========================================================
//    class WeakerPBCModel
//=========================================================
//-----------------------------------------------------------------------
//   static constants
//-----------------------------------------------------------------------

const char *WeakerPBCModel::COARSEN_FACTOR = "coarsenFactor";

//-----------------------------------------------------------------------
//   constructor
//-----------------------------------------------------------------------

WeakerPBCModel::WeakerPBCModel
...
bool WeakerPBCModel::takeAction(const String &action,
...
void WeakerPBCModel::init_(const Properties &globdat)
{
    // Get dofs and cons
...
void WeakerPBCModel::sortBndNodes_()
{
...
template <typename T>
void WeakerPBCModel::sortBndFace_(T &bndFace, const idx_t &index)
{
...
void WeakerPBCModel::findSmallestElement_()
{
    Vector c0(rank_);   // coordinate vector of node "0"
    Vector c1(rank_);   // coordinate vector of node "1"
...
void WeakerPBCModel::createTractionMesh_()
{
...
void WeakerPBCModel::coarsenMesh_(FlexVector &trFace, const idx_t &index)
{
    ...
    for (Iter in = trFace.begin(); in < trFace.end() - 1; ++in)
    {
        ...
        while (norm2(c0 - c1) < min(dx, dx_[index]))
        {
...
void WeakerPBCModel::getTractionMeshNodes_(IdxVector &connect,
                                         const Vector &x,
                                         const idx_t &face)
{
...
void WeakerPBCModel::augmentMatrix_(Ref<MatrixBuilder> mbuilder,
                                  const Vector &fint,
                                  const Vector &solu)
{
...
static Ref<Model> newWeakerPBCModel
...
{
    return newInstance<WeakerPBCModel>(name, conf, props, globdat);
}
...

{
    ...
    ModelFactory::declare("WeakerPBC", &newWeakerPBCModel);
}